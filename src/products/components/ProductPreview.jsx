...
const ProductPreview = ({ product }) => {
  ...
  const accordionData = [
    ...
    },
    {
      id: 2,
      title: "Ingredients",
      desc: "Our Ultimate Double Cheeseburger is made with fresh, high-quality ingredients: two juicy flame-grilled beef patties, melted cheddar cheese, crisp lettuce, ripe tomatoes, tangy pickles, onions, and our signature house-made sauce — all served on a toasted brioche bun.",
    },
    {
      id: 3,
      title: "Shipping",
      desc: "We deliver your order hot and fresh, straight from our kitchen to your door. Average delivery time is 30–45 minutes within the city. For outside areas, delivery may take up to 60 minutes. All burgers are carefully packaged to preserve flavor and freshness during transit.",
    },
  ];
  ...
  return (
    <div className="pt-[5rem] lg:space-x mx-[1rem]">
      {product.map((p, index) => (
        <div
          className="flex items-start lg:flex-row flex-col gap-[1rem]"
          key={index}
        >
          ...
          <div className="flex flex-col gap-[1rem] lg:w-[70%] w-full">
            ...
            <span className="text-[2rem]">${p.price}</span>
            ...
            <div className="flex items-center gap-[1rem] w-full">
              <button
                ...
                className={`${
                  size === "Double" && "!bg-[#ff5227] !text-white"
                }  btn --border-orange !text-[1.2rem] !w-auto inst-sans uppercase !p-[.5rem] !rounded-[2rem]`}
              >
                Double
              </button>
              <button
                ...
                className={`${
                  size === "Large" && "!bg-[#ff5227] !text-white"
                }  btn --border-orange !text-[1.2rem] !w-auto inst-sans uppercase !p-[.5rem] !rounded-[2rem]`}
              >
                Large
              </button>
              <button
                ...
                className={`${
                  size === "Small" && "!bg-[#ff5227] !text-white"
                }  btn --border-orange !text-[1.2rem] !w-auto inst-sans uppercase !p-[.5rem] !rounded-[2rem]`}
              >
                Small
              </button>
            ...
            <Button
              text={"Add to cart"}
              className={"text-[1.5rem] py-[1rem]  my-[1.5rem] px-[2rem]"}
              type={"--border-orange"}
              onClick={() => addToCart(p)}
            />

            <div className="bg-[#f9dff2] rounded-[1rem] p-[1rem]">
              <h2 className="text-[2.2rem] pb-[.5rem]">
                ...
                <div className="flex lg:flex-row flex-col lg:items-center items-start gap-[1rem] justify-between w-full ">
                  <div className="flex items-start gap-[1rem]">
                    <img
                      src={p.img}
                      alt={p.name}
                      className="border border-[#ff5227] rounded-[1rem] w-[6rem] h-[6rem] object-cover"
                    />
                    ...
                    <div>
                      ...
                      <span>
                        {" "}
                        <span className=" font-medium inst-sans">4</span>-Pack /
                        ${p.price}
                      </span>
                    </div>
                  </div>
                  ...
                  <Button
                    ...
                    text={"Add"}
                    className={
                      "w-[7rem] p-[.5rem] lg:text-[1.5rem] text-[1.3rem]"
                    }
                  />
                </div>
            ...
            <div className="flex flex-col gap-[1rem]">
              {accordionData.map((acc, index) => (
                <div
                  key={index}
                  className={`cursor-pointer border-b border-[#ff5227] w-full pb-[1rem] transition-transform ${
                    currentIndex === index && accordion
                      ? "h-auto overflow-auto"
                      : "h-[5rem] overflow-hidden"
                  }`}
                  onClick={() => handleAccordion(index)}
                >
                  <div className="flex items-center justify-between">
                    <h2 className="lg:text-[3rem] text-[2.5rem]">
                      {acc.title}
                    </h2>

                    <button
                      type="button"
                      className="border border-[#ff5227] rounded-full p-[.5rem]"
                    >
                      <Plus />
                    </button>
                  </div>
